...
export const BuilderStepTemplate = () => {
  ...
  return (
    <div className="flex flex-1 h-full">
      {/* Left: Editor */}
      <div className="flex-1 flex flex-col min-w-0 border-r border-[var(--hairline)]">
        {/* Guidance bar */}
        <div className="flex items-center gap-3 px-4 py-2 bg-[var(--warm)]/5 border-b border-[var(--hairline)]">
          <Lightbulb className="h-3.5 w-3.5 text-warm shrink-0" strokeWidth={1.5} />
          <p className="text-[11px] text-muted-foreground">{t('shablonBuilder.templateGuide')}</p>
          <div className="flex-1" />
          <button
            onClick={() => setShowVariables(!showVariables)}
            className={`flex items-center gap-1 px-2 py-1 text-[10px] font-medium rounded-md transition-colors ${
              showVariables ? 'bg-[var(--warm)]/10 text-warm' : 'text-muted-foreground hover:text-foreground'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2 py-1 text-[10px] font-medium rounded-md transition-colors ${
              showPreview ? 'bg-[var(--warm)]/10 text-warm' : 'text-muted-foreground hover:text-foreground'
            }`}
          >
        ...
        <div className="flex-1 flex" style={{ minHeight: showPreview ? 'auto' : 300 }}>
          {showPreview ? (
            <div className="flex-1 overflow-y-auto p-5">
              <div
                className="text-sm leading-relaxed whitespace-pre-wrap font-serif rounded-xl border border-[var(--hairline)] bg-[var(--paper-bg)] p-5 min-h-full"
              ...
              >
                    ...
                    {line.split(/({{.+?}})/g).map((part, j) => {
                      if (part.startsWith('{{') && part.endsWith('}}')) {
                        ...
                        return (
                          <span
                            key={j}
                            className={`px-0.5 rounded ${val ? 'bg-green-500/10 text-green-500' : 'text-warm bg-warm/5'}`}
                          >
                            {val || part}
                          </span>
                        )
                ...
                {!draft.template && (
                  <p className="text-muted-foreground/40 italic">{t('shablonBuilder.previewEmpty')}</p>
                )}
              </div>
            </div>
          ) : (
            <div className="flex-1 flex flex-col p-4">
              ...
              />
              {/* Quick-insert bar */}
              <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-[var(--hairline)] flex-wrap">
                <span className="text-[10px] text-muted-foreground mr-1 shrink-0">{t('shablonBuilder.insertVariable')}:</span>
                {fieldKeys.slice(0, 12).map((key) => {
                  ...
                  return (
                    <button
                      ...
                      className="px-1.5 py-0.5 text-[10px] font-mono bg-[var(--warm)]/5 hover:bg-[var(--warm)]/10 text-warm border border-[var(--warm)]/10 rounded transition-colors"
                    >
                      {'{{' + key + '}}'}
                    </button>
                  )
                ...
                {fieldKeys.length > 12 && (
                  <span className="text-[10px] text-muted-foreground">+{fieldKeys.length - 12} more</span>
                )}
                {fieldKeys.length === 0 && (
                  <p className="text-[10px] text-muted-foreground/50 italic">{t('shablonBuilder.noVariables')}</p>
                )}
              </div>
            </div>
          )}
      ...
      <AnimatePresence>
        {showVariables && (
          ...
          >
            <div className="w-[340px] h-full flex flex-col border-l border-[var(--hairline)] bg-[var(--surface)]">
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--hairline)]">
                <span className="text-xs font-medium text-foreground">{t('shablonBuilder.variables')}</span>
                <span className="text-[10px] text-muted-foreground">{fieldKeys.length} fields</span>
              </div>

              <div className="flex-1 overflow-y-auto p-3 space-y-2">
                {fieldKeys.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    <Beaker className="h-6 w-6 text-muted-foreground/30 mb-2" strokeWidth={1} />
                    <p className="text-[11px] text-muted-foreground">{t('shablonBuilder.noFieldsYet')}</p>
                    <p className="text-[10px] text-muted-foreground/50 mt-1">{t('shablonBuilder.addFieldsGuide')}</p>
                  </div>
                )}
              ...
              {fieldKeys.length > 0 && (
                <div className="border-t border-[var(--hairline)] p-3">
                  <div className="flex items-center gap-1.5 mb-2">
                    <Beaker className="h-3 w-3 text-muted-foreground" strokeWidth={1.5} />
                    <span className="text-[10px] font-medium text-muted-foreground">{t('shablonBuilder.testPreview')}</span>
                  </div>
                  <div
                    className="text-[11px] leading-relaxed whitespace-pre-wrap font-serif rounded-lg border border-[var(--hairline)] bg-[var(--paper-bg)] p-3 max-h-32 overflow-y-auto"
...
function SyntaxHighlightEditor({
  ...
  const highlighted = useMemo(() => {
    return value
      ...
      .replace(/(\{\{(\w+)\}\})/g, (_, full, key) => {
        const field = fieldMap.get(key)
        const color = field
          ? field.type === 'date' ? '#60a5fa'
            : field.type === 'number' || field.type === 'percent' ? '#f59e0b'
            : field.type === 'textarea' ? '#a78bfa'
            : field.type?.includes('select') ? '#34d399'
            : '#f472b6'
          : '#f472b6'
  ...
  const checkAutocomplete = useCallback(() => {
    ...
    // Check if there's a }} after the {{, before cursor
    const between = text.substring(lastOpen + 2)
    const closeBrace = between.indexOf('}}')
    // Only suggest if we haven't closed the braces yet
  ...
  return (
    <div className="relative flex-1 rounded-xl border border-[var(--hairline)] overflow-hidden bg-[var(--surface)]">
      ...
      <textarea
        ...
        spellCheck={false}
        className="relative w-full h-full min-h-[300px] p-3 font-mono text-sm leading-5 bg-transparent text-transparent caret-foreground border-none outline-none resize-none selection:bg-[var(--warm)]/20"
        style={{
          lineHeight: '20px',
          fontSize: '13px',
          fontFamily: "'Cascadia Code', 'JetBrains Mono', 'Fira Code', 'Consolas', monospace",
      ...
      <AnimatePresence>
        {suggestions.length > 0 && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -4 }}
            className="fixed z-50 w-64 rounded-xl border border-[var(--hairline)] bg-[var(--popover)] shadow-xl overflow-hidden"
            style={{
              top: suggestionPos.top + 100,
              left: suggestionPos.left + 40,
            }}
          >
            <div className="px-3 py-1.5 text-[10px] text-muted-foreground border-b border-[var(--hairline)]">
            ...
            {suggestions.map((s, i) => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 text-left transition-colors ${
                  i === suggestionIndex ? 'bg-[var(--warm)]/10' : 'hover:bg-[var(--surface-hover)]'
                }`}
              >
                ...
                <span className="text-xs font-mono text-warm font-medium">{s.key}</span>
                <span className="text-[10px] text-muted-foreground flex-1 truncate">{s.label}</span>
                <span className="text-[9px] px-1 rounded bg-[var(--surface-hover)] text-muted-foreground">{s.type}</span>
              </button>
            ))}
            <div className="px-3 py-1.5 text-[9px] text-muted-foreground/50 border-t border-[var(--hairline)] flex gap-3">
    ...
    <div className="rounded-xl border border-[var(--hairline)] bg-[var(--surface)] overflow-hidden">
      ...
      >
        {expanded ? (
          ...
          <ChevronRight className="h-3 w-3 text-muted-foreground shrink-0" strokeWidth={2} />
        )}
        <span className="text-xs font-mono text-warm font-medium">{fieldKey}</span>
        <span className="text-[10px] text-muted-foreground flex-1 truncate">{field?.label || ''}</span>
        <span className={`text-[9px] px-1 rounded ${
          field?.type === 'date' ? 'bg-blue-400/10 text-blue-400'
            : field?.type === 'number' || field?.type === 'percent' ? 'bg-amber-400/10 text-amber-400'
            : field?.type === 'textarea' ? 'bg-purple-400/10 text-purple-400'
            : field?.type?.includes('select') ? 'bg-emerald-400/10 text-emerald-400'
            : 'bg-pink-400/10 text-pink-400'
        }`}>
          {field?.type || 'text'}
        </span>
        <span className="text-[10px] text-muted-foreground/50">{field?.required ? 'Required' : 'Opt'}</span>
      </button>
          ...
          >
            <div className="p-3 space-y-2">
              <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                <span className="font-mono bg-[var(--surface-hover)] px-1.5 py-0.5 rounded">
                  {'{{' + fieldKey + '}}'}
                </span>
                {field?.autoFill && (
                  <span className="bg-[var(--warm)]/5 text-warm px-1.5 py-0.5 rounded">
                    auto: {field.autoFill}
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-2 py-1.5 text-[11px] bg-transparent border border-[var(--hairline)] rounded-md outline-none text-foreground placeholder:text-muted-foreground/40 focus:border-[var(--warm)]/30 transition-colors"
                />
                <button
                  onClick={onInsert}
                  className="p-1.5 text-muted-foreground hover:text-warm transition-colors"
