...
import {
  User, Bell, Shield, Globe, LogOut, ChevronRight,
  ClipboardCheck, Moon, Info, Trash2,
} from 'lucide-react'
  ...
  const sections = [
    {
      ...
      items: [
        ...
        { icon: Globe, label: 'Rol', value: roleLabels[activeProfile || user?.role || ''] || user?.role },
      ],
    },
    {
      title: 'Sozlamalar',
  ...
  return (
    <div className="animate-fadeIn pb-8">
      ...
      <div className="mx-4 mb-6 flex items-center gap-4 rounded-2xl border border-white/8 bg-white/4 p-4">
        <Avatar name={user?.full_name || 'U'} role={activeProfile || user?.role || 'teacher'} size="lg" />
        <div className="min-w-0 flex-1">
          <p className="text-base font-bold text-zn-text">{user?.full_name}</p>
          <p className="text-xs text-zn-text-muted">{roleLabels[activeProfile || user?.role || ''] || user?.role}</p>
        ...
        <div key={section.title} className="mb-5">
          ...
          <div className="mx-4 overflow-hidden rounded-2xl border border-white/6 bg-white/4">
            {section.items.map((item, idx) => {
              ...
              return (
                <div key={item.label}
                  className={`flex items-center gap-3 px-4 py-3 ${idx < section.items.length - 1 ? 'border-b border-white/6' : ''}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {item.value && <p className="text-xs text-zn-text-faint">{item.value}</p>}
                  </div>
